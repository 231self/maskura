...
//! with `just bench-plugins` (builds the components first) or directly with
//! `cargo run --release -p maskura-wasm-runtime --example bench_plugins`.

use std::hint::black_box;
use std::path::PathBuf;
use std::time::{Duration, Instant};

use maskura_wasm_runtime::{FilterEngine, Operation, RuntimeLimits, Session, TransformOutcome};
...
fn read_component(name: &str) -> Vec<u8> {
    ...
    std::fs::read(&path).unwrap_or_else(|error| {
        panic!(
            "failed to read {}: {error}; run `just build-plugins` first",
            path.display()
        )
    })
}
