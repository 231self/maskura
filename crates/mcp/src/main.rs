...
mod tests {
    ...
    fn test_server() -> MaskuraServer {
        MaskuraServer::new(
            Config::new(
                "http://localhost:8080",
                Some("maskura_mcp_test".to_string()),
                None,
                None,
            )
            .unwrap(),
...
<ListBucketResult>
  ...
  <CommonPrefixes><Prefix>dir/nested&amp;/</Prefix></CommonPrefixes>
    ...
    #[test]
    fn publishes_only_canonical_tool_names() {
    ...
    async fn tools_preserve_mcp_auth_and_processing_semantics() {
        ...
        let server = MaskuraServer::new(
            Config::new(&gateway_url, Some("maskura_mcp_test-token".to_string()), None, None).unwrap(),
        )
        .unwrap();
        ...
        assert_eq!(requests[0].headers["x-maskura-mcp-token"], "maskura_mcp_test-token");
    ...
    async fn api_key_auth_uses_both_gateway_headers() {
        ...
        let server = MaskuraServer::new(
            Config::new(
                &gateway_url,
                None,
                Some("maskura_access".to_string()),
                Some("maskura_secret_secret".to_string()),
            )
            .unwrap(),
        )
        .unwrap();
        ...
        let requests = state.requests.lock().unwrap();
        assert_eq!(requests[0].headers["x-maskura-access-key"], "maskura_access");
        assert_eq!(requests[0].headers["x-maskura-secret-key"], "maskura_secret_secret");
