...
- **Declarative pipelines (self-hosted)**: a signed TOML file (`MASKURA_PIPELINES_FILE`) replaces catalog-order selection with hierarchical, ordered `write`/`read` chains per workspace/bucket. Schema + Ed25519/CBOR signing live in `crates/pipeline-config/`; the resolver is `crates/gateway/src/pipeline_config.rs`; `maskura pipelines {init-key,sign,verify,check}` authors it. See ADR 0021.
